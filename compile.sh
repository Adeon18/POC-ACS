...
if [[ "$debug_build" == true ]]; then
  ...
  (
    ...
    echo Compiling...
    cmake -DCMAKE_BUILD_TYPE=Debug -DDEQUE_STATS=${enable_deque_stats} -DCMAKE_INSTALL_PREFIX="${install_prefix}" .. || exit 1
    cmake --build . || exit 1
...
if [[ "$optimize_build" == true ]]; then
  ...
  (
    ...
    echo Compiling...
    cmake -DCMAKE_BUILD_TYPE=Release -DDEQUE_STATS=${enable_deque_stats} -DCMAKE_INSTALL_PREFIX="${install_prefix}" .. || exit 1
    cmake --build . || exit 1
    cmake --install . || exit 1
    popd
  )
